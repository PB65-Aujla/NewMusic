...
keyboard = InlineKeyboardMarkup(
    [
        [
            InlineKeyboardButton("📽️ ᴠᴘʟᴀʏ 📺", callback_data="vplay_data"),
        ]
    ]
)
    ...
    try:
        with requests.Session() as s:
            ...
            if video_list:
                ...
                thumbnail = (
                    random_video.find("div", class_="thumb").find("img").get("src")
                )
                if thumbnail:
                    # Replace the size in the thumbnail URL to get 500x500
                    thumbnail_500 = thumbnail.replace("/h", "/m").replace(
                        "/1.jpg", "/3.jpg"
                    )
                    link = (
                        random_video.find("div", class_="thumb-under")
                        .find("a")
                        .get("href")
                    )
                    if (
                        link and "https://" not in link
                    ):  # Check if the link is a valid video link
                        return {
                            "link": "https://www.xnxx.com" + link,
                            "thumbnail": thumbnail_500,
                        }
